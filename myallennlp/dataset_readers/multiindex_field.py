...
class MultiIndexField(Field[torch.Tensor]):
    """
    ...
    def get_padding_lengths(self) -> Dict[str, int]:  # pylint: disable=no-self-use
        return {self._label_namespace+"num_indexes":self._sequence_length }
    ...
    def empty_field(self):
        return MultiIndexField([], self._label_namespace)

    def __str__(self) -> str:
        return f"MultiIndexField with indexes: {self.labels} in namespace: '{self._label_namespace}'.'"
