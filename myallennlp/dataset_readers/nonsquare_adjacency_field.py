...
class NonSquareAdjacencyField(Field[torch.Tensor]):
    """
    ...
    def __init__(self,
                 indices: List[Tuple[int, int]],
                 sequence_field: SequenceField,
                 head_index_filed: MultiIndexField,
                 labels: List[str] = None,
                 label_namespace: str = 'labels',
                 padding_value: int = -1) -> None:
        self.indices = indices
        self.labels = labels
        self.head_index_filed = head_index_filed
        ...
        if not all([0 <= index[1] < field_length and 0 <= index[0] < field_length for index in indices]):
            raise ConfigurationError(f"Label indices and sequence length "
                                     f"are incompatible: {indices} and {field_length} and {labels}")
    ...
    def get_padding_lengths(self) -> Dict[str, int]:
        return {'num_tokens': self.sequence_field.sequence_length(),self._label_namespace+"num_heads":self.head_index_filed.sequence_length()}
    ...
    @overrides
    def empty_field(self) -> 'NonSquareAdjacencyField':
        # pylint: disable=protected-access
        # The empty_list here is needed for mypy
        empty_list: List[Tuple[int, int]] = []
        adjacency_field = NonSquareAdjacencyField(empty_list,
                                         self.sequence_field.empty_field(),
                                         self.head_index_filed.empty_field(),
