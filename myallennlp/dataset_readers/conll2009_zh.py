...
class PropbankReader:
    ...
    def add_lemma(self, node):
        ...
        for child in node:
            if child.tag == "id":
                lemma = child.text.replace("_", "-").strip()
            if child.tag == "frameset":
                sense = child.attrib["id"].replace("f",".0")
                if sense not in senses:
                    senses.append(sense)

        if len(senses) > 0 and lemma is not None:
...
@DatasetReader.register("conll2009_zh")
class Conll2009ZhDatasetReader(Conll2009DatasetReader):
    """
...
def main():
    data_folder = "/afs/inf.ed.ac.uk/group/project/xchen13/datasets/2009_conll_p2/data/CoNLL2009-ST-Chinese/"
    reader = Conll2009ZhDatasetReader(data_folder = data_folder,read_frame_new=True)

    train_data = reader.read(data_folder+"CoNLL2009-ST-Chinese-train.txt")
    dev_data = reader.read(data_folder+"CoNLL2009-ST-Chinese-development.txt")

    reader.save_frames()

    reader = Conll2009ZhDatasetReader(data_folder = data_folder)

    dev_data = reader.read(data_folder +"CoNLL2009-ST-evaluation-Chinese.txt")
