    ...
    def add_lemma(self, node):
        ...
        for child in node:
            if child.tag == "roleset":
                ...
                if sensed_predicate not in self.frames.setdefault(lemma, []):
                    self.frames[lemma].append(sensed_predicate)
...
def parse_sentence(sentence_blob: str) -> Tuple[List[Dict[str, str]], List[Tuple[int, int]], List[str]]:
    ...
    for line_idx, line in enumerate(lines):
        ...
        if "fillpred"   not in annotated_token:
            print (annotated_token,line_idx,"\n",sentence_blob)
            assert False
...
@DatasetReader.register("conll2009_jp")
...
def main():
    ...
    train_data = reader.read(data_folder+"CoNLL2009-ST-Japanese-train.txt")
    dev_data = reader.read(data_folder+"CoNLL2009-ST-Japanese-development.txt")

    reader.save_frames()

    #reader = Conll2009DeDatasetReader(data_folder = data_folder)
    #test_data = reader.read(data_folder +"CoNLL2009-ST-evaluation-Japanese.txt")
