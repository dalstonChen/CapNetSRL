...
@Model.register("advanced_srl_capsule")
class SRLGraphParserBase(Model):
    ...
    def __init__(self,
                 ...
                 comments: str = "",
                 non_linear_for_argument_cap: bool = False,
                 cap_activation: str = "squash") -> None:
        super(SRLGraphParserBase, self).__init__(vocab, regularizer)
        self.tanh = torch.nn.Tanh()
        self.cap_activation=cap_activation
        if cap_activation != "squash":
            self.cap_activation = Activation.by_name(cap_activation)()
        self.non_linear_for_argument_cap = non_linear_for_argument_cap
        ...
        if global_node == True:
            ...
            self.get_global_layer2 = Plain_Feedforward((num_labels + 1) * capsule_dim, capsule_dim,
                                                      Activation.by_name('relu')())
            self.bilinear_matrix_capsule_layer_for_global_node2 = BilinearMatrix(capsule_dim, capsule_dim)
     ...
     #   print ("arc_types",self.vocab._index_to_token["arc_types"])
        ...
        if self.bilinear_matrix_capsule == True:
            ...
            self.bilinear_matrix_capsule_layer2 = BilinearMatrix(capsule_dim, capsule_dim)
    ...
    def capsule_net_layer(self, layer_u, iter_num):
        ...
        for iter in range(iter_num):
            ...
            if self.cap_activation == "squash":
                v = squash(s)
            else:
                v = self.cap_activation(s)

            # l shape: (batch_size, sequence_length, num_tags + 1, capsule_dim)
            l = torch.sum(layer_u * c.unsqueeze(-1), 2)
            if self.non_linear_for_argument_cap == True:
                l = self.tanh(l)

            # new_b shape: (batch_size, sequence_length, predicates_len, num_tags + 1)
            if self.bilinear_matrix_capsule == False:
                new_b = torch.sum(v.unsqueeze(1) * layer_u, -1) + torch.sum(l.unsqueeze(2) * layer_u, -1)
            else:
                new_b = self.bilinear_matrix_capsule_layer(v.unsqueeze(1), layer_u) + self.bilinear_matrix_capsule_layer2(l.unsqueeze(2), layer_u)
            ...
            if self.global_node == True:
                ...
                batch_size, seq_len, _, capsule_dim = l.size()
                lll = l.view(batch_size, seq_len, -1)
                glo = self.get_global_layer2(lll)
                new_b += self.bilinear_matrix_capsule_layer_for_global_node2(layer_u, glo.unsqueeze(-2).unsqueeze(2))
            b = b + new_b
        return b
