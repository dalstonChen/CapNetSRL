...
@Model.register("srl_capsule_multi")
class SRLGraphParserBase(Model):
    ...
    def __init__(self,
        ...
        self.get_global_layer = Plain_Feedforward((num_labels + 1) * capsule_dim, arc_representation_dim, Activation.by_name('relu')())
     ...
     #   print ("arc_types",self.vocab._index_to_token["arc_types"])
        ...
        self.tag_bilinear = BilinearMatrixAttention_Lowrank(tag_representation_dim*2,
    ...
    def capsule_net_layer_with_massage_passing(self, layer_u, iter_num, att_type):
        ...
        for iter in range(iter_num):
            ...
            b = b + new_b
        return b
    def capsule_net_layer_multi(self, arg_tag_representation, pred_tag_representation, glo_nodes, padding_for_predicate, effective_predicate_indexes, iter_num, predicate_indexes):
        batch_size, sequence_length,_ = arg_tag_representation.size()
        _,predicates_len,_ = pred_tag_representation.size()
        b = numpy.zeros((batch_size, sequence_length, predicates_len, self.num_labels + 1), dtype=float)
        b = torch.from_numpy(b).float().to(arg_tag_representation.device)
        ## shape: (batch_size, sequence_length + 1, dim)
        #glo_nodes_pred = torch.cat([padding_for_predicate, glo_nodes], dim=1)
        ## shape: (batch_size, predicates_len, dim)
        #glo_nodes_pred = glo_nodes_pred.gather(dim=1, index=effective_predicate_indexes)
        glo_nodes = torch.zeros(arg_tag_representation.size()[0],arg_tag_representation.size()[1] + 1,arg_tag_representation.size()[2]).float().to(arg_tag_representation.device)
        glo_nodes_pred = torch.zeros_like(pred_tag_representation).float().to(arg_tag_representation.device)
        for iter in range(iter_num):
            # shape (batch_size, num_tags * capsule_dim, sequence_length, predicates_len)
            #print(glo_nodes[:,1:,:].device,glo_nodes.device,b.device)
            arc_tag_logits = self.tag_bilinear(torch.cat([arg_tag_representation,glo_nodes[:,1:,:]],dim = -1),
                                               torch.cat([pred_tag_representation,glo_nodes_pred], dim = -1))
            # Switch to (batch_size, sequence_length, predicates_len, num_tags, capsule_dim)
            layer_u = arc_tag_logits.permute(0, 2, 3, 1).view(batch_size, sequence_length, -1,
                                                                     self.vocab.get_vocab_size("arc_types") + 1,
                                                                     self.capsule_dim)
            ...
            def cal_new_global(local, v, att_type):
                batch_size, predicates_len, _, capsule_dim = v.size()
                if att_type == 'plain':
                    v = v.view(batch_size, predicates_len, -1)
                    glo = self.get_global_layer(v)
                elif att_type == 'attention':
                    local = torch.sum(local * c.unsqueeze(-1), -2,keepdim=True) * torch.ones_like(local).to(local.device)
                    #tmp = torch.cat((torch.ones_like(local).to(v.device),torch.ones_like(local).to(v.device)), -1)
            ...
            #print('v size:', v.size())
            new_glo = cal_new_global(layer_u, v, self.passing_type)
            glo_nodes_pred += new_glo

            if self.multi_both == True:
                #print(glo_nodes.size(), predicate_indexes.size(), new_glo.size())
                #predicate_indexes: (batch, predicate_len)
                #glo_nodes: (batch, sentence_len, representation_dim)
                #mask: (batch, sentence_len)
                #mask = torch.zeros((glo_nodes.size()[0],glo_nodes.size()[1]))
                #mask[torch.arange(glo_nodes.size()[0]),predicate_indexes[torch.arange(glo_nodes.size()[0]),:]] = 1
                #print(mask.size())
                #mask = torch.nn.functional.one_hot(predicate_indexes, num_classes = glo_nodes.size()[1]).sum(dim = 1)
                #print(predicate_indexes.unsqueeze(-1).expand_as(glo_nodes_pred).size())
                #print(predicate_indexes.unsqueeze(-1).expand_as(glo_nodes_pred))
                glo_nodes.scatter_add_(dim = 1,index = (predicate_indexes.unsqueeze(-1)+1).expand_as(glo_nodes_pred), src = new_glo)
  ...
  #      print("predicate_indexes", predicate_indexes.size())
    ...
    #    print ("selected encoded_text_for_predicate",encoded_text_for_predicate.size())
        ...
        glo_nodes = torch.zeros((batch_size, sequence_length, self.capsule_dim))

        arc_tag_logits = self.capsule_net_layer_multi(arg_tag_representation, pred_tag_representation, glo_nodes, padding_for_predicate, effective_predicate_indexes, self.iter_num, predicate_indexes)
