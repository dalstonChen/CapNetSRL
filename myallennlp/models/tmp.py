...
from myallennlp.modules.bilinear_matrix_attetion_low_rank import BilinearMatrixAttention_Lowrank
...
@Model.register("srl_capsule_none_tmp")
class SRLGraphParserBase(Model):
    ...
    def __init__(self,
     ...
     #   print ("arc_types",self.vocab._index_to_token["arc_types"])
        ...
        self.tag_bilinear = BilinearMatrixAttention_Lowrank(tag_representation_dim,
                                                    tag_representation_dim,
                                                    rank,
                                                    label_dim=(num_labels + 1) * capsule_dim,
    ...
    def forward(self,  # type: ignore
     ...
     #       self.freeze_initial()
        ...
        padding_for_predicate = encoded_text.mean(dim = 1, keepdim = True)

        # shape (batch_size, predicates_len, hidden_dim)
        encoded_text = torch.cat([padding_for_predicate, encoded_text], dim=1)
        encoded_text_for_predicate = encoded_text
  ...
  #      print("predicate_indexes", predicate_indexes.size())
    ...
    #    if self.training is False and False:
        ...
        arc_tag_logits = arc_tag_logits.permute(0, 2, 3, 1).view(batch_size, sequence_length + 1, -1, self.vocab.get_vocab_size("arc_types") + 1, self.capsule_dim)
        ...
        if arc_tags is not None:
            soft_tags = torch.zeros(size=arc_tag_logits[:,1:,:,:].size(), device=arc_tag_logits.device)
        ...
        if arc_tags is not None:
            loss = self._construct_loss(arc_tag_logits,
                                        ...
                                        predicate_mask,
                                        float_mask)
            self._labelled_f1(arc_tag_probs[:,1:,:,:], arc_tags + 1, graph_mask.squeeze(-1),
                          sense_probs, predicate_candidates,
                          predicates, linear_scores=arc_tag_probs[:,1:,:,:] * arc_tag_logits[:,1:,:,:], n_iteration=1)

        else:
            loss = 0

        arc_tag_probs = arc_tag_probs[:,1:,:,:]
    ...
    def _construct_loss(self, arc_tag_logits,
        ...
        float_mask = torch.cat([torch.ones(float_mask.size()[0], 1, device=float_mask.device), float_mask], -1)

        # shape (batch ,sequence_length,predicates_len ,1)
        delta_tag = self._tag_loss(torch.nn.functional.log_softmax(arc_tag_logits[:,1:,:,:], dim=-1).permute(0, 3, 1, 2),
        ...
        delta_tag_second = self._tag_loss(
            MaskedOperation.log_softmax(arc_tag_logits[:, :, :, 1:], dim=1, mask=float_mask.unsqueeze(-1).unsqueeze(-1)),
            torch.argmax(soft_tags[:, :, :, 1:], 1)).unsqueeze(1) * graph_mask
        ...
        second_loss = 0 if self.double_loss==False else delta_tag_second.sum() / (predicate_mask.sum().float() * (soft_tags.size()[-1] - 1))
        if self.delta_type == "kl_only":

            return (delta_tag.sum() + delta_sense.sum() )/ valid_positions + second_loss# + arc_nll
